...
#+begin_src C++
  ...
  int keyWrapper(vector<uint8_t> &kek, vector<uint8_t> &key, vector<uint8_t> &wrappedKey)
  {
      ...
      int n = key.size() / 8;
      uint8_t R[n+1][8] = {0,};
      vector<uint8_t> A = hex_decode("A6A6A6A6A6A6A6A6");
  ...
  int keyUnwrapper(vector<uint8_t> &kek, vector<uint8_t> &wrappedKey, vector<uint8_t> &key)
  {
      ...
      std::vector<uint8_t> originalKey(R[1], R[1]+16);
      key.clear();
      key.assign(originalKey.begin(), originalKey.end());
