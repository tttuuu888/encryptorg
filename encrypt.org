...
#+begin_src C++
    ...
    int main ()
    {
        string key1 = KEY;
        string msg1 = MSG;
        std::vector<uint8_t> key(key1.begin(), key1.end());
        std::vector<uint8_t> msg(msg1.begin(), msg1.end());
...
#+begin_src C++
  ...
  int main ()
  {
      string plain = PASSPHRASE;
      vector<uint8_t> label = hex_decode(INFO);
      vector<uint8_t> salt = hex_decode(SALT);
      vector<uint8_t> passphrase(plain.begin(), plain.end());

      span secret{passphrase.data(), passphrase.size()};

      auto kdf = Botan::KDF::create_or_throw("HKDF(SHA-256)");
      auto key = kdf->derive_key(KEY_LEN, secret, salt, label);
...
#+RESULTS:
: Derived key: 7B07FAB24B269299C42E6E8ED99760324CD302BC5D0BD6CCE45896C7E1BAD533
...
#+begin_src C++
  ...
  int main ()
  {
      ...
      vector<uint8_t> label = hex_decode(INFO);
      vector<uint8_t> salt = hex_decode(SALT);
      vector<uint8_t> passphrase = hex_decode(PASSPHRASE);
...
#+RESULTS:
: Derived key: 11E806DCCE73A454CB16AF7EE68F567C5DCD568F305459503A59D0BA17EF970A
** PBKDF2
